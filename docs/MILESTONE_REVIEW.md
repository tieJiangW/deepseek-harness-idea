...
> 旧 session 迁移、API key 全局化等）、**v0.2.0 / v0.2.1**（瘦身 + 按平台下载运行时模型、下载可靠性与 UX）
> 与 **v0.2.2 / v0.2.3**（2024.x 描述符修复；dsh 0.1.5-rc.2 升级与鉴权/RPC 适配；五平台运行时；设置页与文件选择器修复
> ——当前插件版本 **0.2.3**，2026-09-16）均已远超本快照。**最新状态请看 [README.md](./README.md) 的「当前状态摘要」**，
> 实现细节见 [DESIGN.md](./DESIGN.md) / [PROJECT_NOTES.md](./PROJECT_NOTES.md) / [ACCEPTANCE.md](./ACCEPTANCE.md)；
> **本文件 §3 的遗留问题清单（A–D）仍在维护使用**，其余部分仅作历史评审背景留存。
...
| Marketplace 上架 | ✅ 已进入上架流程（快照时点未做）；**v0.2.3 上传 JetBrains Marketplace——update id 1171727，待审核**（v0.2.1 为 1159301） |
...
| A-4 | 发送选中代码 / 日志一键解释的 JCEF 注入效果（含剪贴板降级） | PRD §7-5 / US-06 | 队列/工具链路自动化；**注入脚本基于 `<textarea>` composer 编写，而 dsh 0.1.5 已改为 Lexical contenteditable → 当前必然降级为剪贴板，待适配后复验**（见 PROJECT_NOTES「dsh 0.1.5-rc.2 升级」） |
...
| C-2 | dsh 版本升级流程 | 升级 = 改 `DSH_VERSION` + 重建（或交叉构建）运行时 + 回归启动/鉴权/RPC/patch 语法。**v0.2.3 已完成一次跨版本升级：0.1.1-rc.2 → 0.1.5-rc.2**（三处契约变更见 PROJECT_NOTES「dsh 0.1.5-rc.2 升级」）；当前锁定 **0.1.5-rc.2** |
...
| C-5 | 版本号口径不一致 | ✅ 已解决：v0.2.0 起按 0.2.x 发版对齐；**当前插件版本 0.2.3（2026-09-16）**，已发布 GitHub Release（11 资产）与 Marketplace（update 1171727） |
...
- **v0.2.2（2026-09-03）**：修复 IntelliJ 2024.x 启动报 `contains invalid plugin descriptor`——根因是 `plugin.xml`
  根级 `<icon>` 不被 2024.x 描述符解析器识别（`Unknown element: icon` → 启动期 `Logger.error` 抛异常）；
  移除该元素即可，工具窗口/动作的子元素 `icon=` 属性不受影响。测试 122/122 保持通过。
- **v0.2.3（2026-09-16，当前）**：dsh 运行时 **0.1.1-rc.2 → 0.1.5-rc.2**（Node 22.23.2）+ 三处契约适配
  （启动 URL 浏览器鉴权 token→cookie；RPC 斜杠命名空间 + `{"payload":{"args":{"request":…}}}` 信封；
  `workspace/list` 移除 → 工作区顺序改读 `storages/workspace.json` v2）；**五平台运行时**（新增 macOS x64 与 Linux arm64，
  `build-runtime.mjs` 支持在 Windows 上交叉构建 Unix 运行时——符号链接容错 / 主机 node 执行 npm / `--libc glibc`）；
  设置页「运行时下载地址」反显 + 默认按钮、新增「运行时目录」设置项（默认按钮填入插件默认目录且等价于未设置）、
  本地 zip 选择器修复（FileChooser `chooseJars`）与 VFS 暂存导入、错误卡换行与诊断增强；
  IDE 边界 2024.1.7 / 2024.3.2 / 2026.2 编译（含测试代码）全部通过；测试 **130/130 全部通过（0 失败）**；
  产物 `deepseek-harness-idea-0.2.3.zip`（≈1.83MB）；GitHub Release v0.2.3（11 资产）+ Marketplace update 1171727（待审）。
...
- 平台扩展：**✅ 已完成（v0.2.3）**：五平台（win-x64 / macos-arm64 / macos-x64 / linux-x64 / linux-arm64）运行时均已构建并随 Release 发布；
  其中 **macos-x64（Intel Mac，GitHub 无该平台 runner）与 linux-arm64 由 Windows 主机交叉构建后手动上传**
  （`scripts/build-runtime.mjs` 已支持，见 [release-runtime.md](./release-runtime.md) §3.1）。
...
- **遗留问题已分级归档**（A 验收 / B 功能 / C 技术债 / D 文档债），可作为后续迭代的 backlog 直接使用。
- **后续进展（截至 v0.2.3，2026-09-16）**：A 类手工验收仍待真实 IDE 会话（安装 / 真实对话 / diff UI / 注入 / 进程清理）；
  C-5 已解决、C-2 已完成一次跨版本升级（→ 0.1.5-rc.2）并沉淀出流程；
  **当前最高优先事项是 dsh 0.1.5 composer 改 Lexical 导致的 JS 注入降级（A-4）**，其次为 B-1「发送当前文件」、
  B-2 远程开发检测、B-3 `ide_reveal_file` 增强与 C-1（intellij 2.x 升级）。
